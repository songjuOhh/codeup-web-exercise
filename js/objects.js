(function() {
    ...
    function findCoffee(input){
        let coffeeNew = [], name , lowered;
        for (let i=0; i<coffees.length; i++) {
            lowered = input.toLowerCase();
            if (coffees[i].name.toLowerCase().includes(lowered) ||
                coffees[i].roast.toLowerCase().includes(lowered)) {
                coffeeNew.push({name: coffees[i].name, roast: coffees[i].roast});
            }
        }
        return coffeeNew;
    }
    console.log(findCoffee('l'));
